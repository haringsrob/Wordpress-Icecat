 ...
 * Description: This plugin allows you grab icecat data and attach it to your post
 * Author: tortelduif
 * Version: 4.2
...
function icecat_getdata($content, $newcontent = FALSE, $import = FALSE) {
  ...
  if ($icecat->hasErrors()) {
    ...
    $_SESSION['IcecatNotice'][] = $icecat->hasErrors();
    return;
  }
  ...
  if (is_plugin_active('woocommerce/woocommerce.php') && get_option('icecat_set_category')=="on") {
    if (function_exists('sanitize_title') && !term_exists($category, 'product_cat')) {
      ...
      $product_category_id = get_term_by('name', $category, 'product_cat'); // Get our term id by name
      $update = true;
      // The category is created, update is set to true so we can add it to our product.
    }
    else {
      if (!has_category( $category, $post )) {
        // Category must be linked becouse it allready exists.
        $product_category_id = get_term_by('name', $category, 'product_cat'); // Get our term id by name
        $update = true; // Tell to update the post!
      }
    }
  }
  ...
  if(is_array($images) && get_option('icecat_download_images')=="on" && $post->post_type=="product" && !has_post_thumbnail( $post->ID )){
    ...
    if (is_plugin_active('woocommerce/woocommerce.php') && isset($images[0]['high'])) {
      ...
      unset($images[0]);
      // Skip downloading additional images if it is an import. Would take a long time.
      if (!$import || get_option('icecat_wp_all_import_multiimage')=='on') {
        ...
        foreach ($images as $pic) {
          ...
          if (is_wp_error($tmp)) {
            $_SESSION['IcecatNotice'][] = array('message' => 'Error saving an image: <strong>' . $tmp->get_error_message() . '</strong>', 'type' => 'error');
            break;
          }
          // Match the filename.
  ...
  if (is_array($specs)) {
    foreach($specs as $spec){
      ...
      if (get_option('icecat_specs_body')=="on") {
        ...
        $appendtobody .= '<td class="feature">'.$spec['name'].'</td><td class="featurevalue">'.$spec['data'].'</td>';
        $appendtobody .= '</tr>';
      }
      // Reset if 2;
      if($i==2){
...
function icecat_save_postdata( $post_id ) {
  if ($_POST) { // Check if it is a post!
    //verify if this is an auto save routine.
    if (defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE){ return; }
    //verify this came from the our screen and with proper authorization,
    if (empty($_POST['icecat_noncename']) ||
         !wp_verify_nonce( $_POST['icecat_noncename'], plugin_basename( __FILE__ ) )
       ){
      return;
    }
    // Check permissions
